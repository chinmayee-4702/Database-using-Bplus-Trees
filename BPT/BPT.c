...
record * make_record(int value,int marks,char*name) {
	...
	if (new_record == NULL) {
		...
		new_record->MIS = value;
		new_record->marks = marks;
		new_record->name = (char*)malloc(sizeof(char)*s);
...
node * insert_into_leaf( node * leaf, int key, record * pointer ) {
	...
	while (insertion_point < leaf->num_keys && leaf->keys[insertion_point] < key)
		insertion_point++;
	//Emptifying the bloc for new key
...
node * insert_into_leaf_after_splitting(node * root, node * leaf, int key, record * pointer) {
	...
	while (insertion_index < order - 1 && leaf->keys[insertion_index] < key)
		insertion_index++; //finding point of insertion

	for (i = 0, j = 0; i < leaf->num_keys; i++, j++) {
		if (j == insertion_index) j++;
		temp_keys[j] = leaf->keys[i];
		temp_pointers[j] = leaf->pointers[i]; //emptified a block for new key in temp array
	}

	temp_keys[insertion_index] = key; //Refilling the emptifies block in temp array
	temp_pointers[insertion_index] = pointer; //Assigning the pointer
...
node * insert( node * root, int key, int value,int marks,char*name ) {
	 ...
	 * value.
	 */
	pointer = make_record(value,marks,name);
...
node * remove_entry_from_node(node * n, int key, node * pointer) {//here n is the pointer to a particular node

	int i, num_pointers;

	// Remove the key and shift other keys accordingly.
	i = 0;
	while (n->keys[i] != key)//Reach the appropriate key
		i++;
	...
	num_pointers = n->is_leaf ? n->num_keys : n->num_keys + 1;//checking whether node is leaf node or internal node
	i = 0;
	while (n->pointers[i] != pointer)
		i++;//Searching for pointer to be deleted
	for (++i; i < num_pointers; i++)//once the pointer is searched ,delete it and shift other pointers accordingly
		n->pointers[i - 1] = n->pointers[i];
	...
	if (n->is_leaf)
		for (i = n->num_keys; i < order - 1; i++)//traverse the node
			n->pointers[i] = NULL;
	else
...
node * adjust_root(node * root) {
	...
	if (root->num_keys > 0)//checkes conditions for non empty root
		return root;
	...
	if (!root->is_leaf) {
		new_root = root->pointers[0];//first child
...
node * coalesce_nodes(node * root, node * n, node * neighbor, int neighbor_index, int k_prime) {
	 ...
	 * in the special case  if n is a leftmost child.
	 */
...
node * redistribute_nodes(node * root, node * n, node * neighbor, int neighbor_index, 
	...
	if (neighbor_index != -1) {
		...
		for (i = n->num_keys; i > 0; i--) {
			n->keys[i] = n->keys[i - 1];//shift keys to right
			n->pointers[i] = n->pointers[i - 1];//shift pointer to right
		}
		if (!n->is_leaf) {
			n->pointers[0] = neighbor->pointers[neighbor->num_keys];//rightmost ptr of neighbor is shifted to leftmost of node
			tmp = (node *)n->pointers[0];
			tmp->parent = n;//the content of rightmost ptr of neighbor is saved 
			neighbor->pointers[neighbor->num_keys] = NULL;//it is pointing to NULL
			n->keys[0] = k_prime;//parent of rightmost key of neighbor k_prime
			n->parent->keys[k_prime_index] = neighbor->keys[neighbor->num_keys - 1];
		}
		else {
			n->pointers[0] = neighbor->pointers[neighbor->num_keys - 1];//last ptr is occupied
	 ...
	 */
		...
		if (n->is_leaf) {//leaf node
			...
			tmp = (node *)n->pointers[n->num_keys + 1];//temperory pointer
			tmp->parent = n;
			n->parent->keys[k_prime_index] = neighbor->keys[0];
		}
		for (i = 0; i < neighbor->num_keys; i++) {//shift the position of keys and pointer
...
void find_and_print_range( node * root, int key_start, int key_end,
	...
	if (!num_found)
		...
		for (i = 0; i < num_found; i++)
			printf("MIS: %d  Marks: %d  Name : %s\n",returned_keys[i],((record *)returned_pointers[i])->marks,((record *)returned_pointers[i])->name);
	}
}
